/**
  ...
  *  V2.0.0     Nov-11-2019     RM              1. support bmi088, but don't
  support mpu6500
  *
  @verbatim
  ==============================================================================
...
void INS_task(void const *pvParameters) {
  ...
  while (1) {
    ...
    if (accel_temp_update_flag & (1 << IMU_UPDATE_SHFITS)) {
      ...
      BMI088_temperature_read_over(accel_temp_dma_rx_buf +
                                       BMI088_ACCEL_RX_BUF_DATA_OFFSET,
                                   &bmi088_real_data.temp);
      imu_temp_control(bmi088_real_data.temp);
    }

    AHRS_update(INS_quat, 0.001f, bmi088_real_data.gyro, bmi088_real_data.accel,
                ist8310_real_data.mag);
    get_angle(INS_quat, INS_angle + INS_YAW_ADDRESS_OFFSET,
              INS_angle + INS_PITCH_ADDRESS_OFFSET,
              INS_angle + INS_ROLL_ADDRESS_OFFSET);
    broadcast_buffer_update(ANGLE_RADIAN, INS_quat[0], INS_quat[1], INS_quat[2],
                            INS_quat[3], INS_angle[INS_ROLL_ADDRESS_OFFSET],
                            INS_angle[INS_PITCH_ADDRESS_OFFSET],
...
static void imu_cmd_spi_dma(void) {
  ...
  if ((accel_update_flag & (1 << IMU_DR_SHFITS)) &&
    ...
    SPI1_DMA_enable((uint32_t)accel_dma_tx_buf, (uint32_t)accel_dma_rx_buf,
